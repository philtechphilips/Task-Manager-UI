...
//   const [image, setImage] = useState("");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.patch("user/me", { name, email, age },
        {
          headers: {
            Authorization: `Bearer ${token}`, // set Authorization header with token value
          },
      ...
      <div className="w-full flex flex-wrap justify-around">
        ...
        >
          ...
          <div className="mb-4">
            ...
            />
            {errors.age && (
            <p className="text-red-600">{errors.age.properties.message}</p>
          )}
          </div>
        ...
        {/* <form
          ...
          className="w-96 mt-20 mb-20 pr-4 pl-4"
        >
          <h1 className="text-center font-bold text-2xl mb-3">Update Profile Image</h1>
          {success && <p className="text-green-600 text-center">{success}</p>}
          <div className="mb-4">
            <label
              htmlFor="description"
          ...
          <button
            ...
            className="bg-blue-500 w-full hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
          >
            {isSubmitting ? <Spinner /> : "Update Profile Image"}
          </button>
        </form>
        */}
      </div>
